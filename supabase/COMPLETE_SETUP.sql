...
-- Single-shot script. Run ONCE in Supabase SQL Editor.
-- Safe to re-run (fully idempotent).
-- Version: FINAL | Built by NayakWorks
-- ============================================================
...
-- ============================================================
-- PRE-FLIGHT: DROP ALL EXISTING FUNCTIONS & TRIGGERS
-- PostgreSQL cannot change return types via CREATE OR REPLACE.
-- Dropping first makes this script fully re-runnable.
-- ============================================================

-- Drop trigger functions (must drop triggers first, then functions)
...
-- Drop all functions (CASCADE removes any dependent objects)
...
-- ============================================================
-- SECTION 1: ALL TABLES (idempotent)
-- ============================================================
...
-- 34. used_utrs (fraud prevention — track every UTR that was used)
...
-- ============================================================
-- SECTION 2: ADD MISSING COLUMNS (safe ALTER TABLE)
-- ============================================================
...
-- Fix constraints (drop old, add new flexible ones)
ALTER TABLE subscription_plans
...
-- Student code generator (per-tenant: PREFIX + padded number)
...
-- Receipt number generator (per-tenant: PREFIX-YYYYMM-NNNN)
...
-- Sync seat → occupied/available when membership changes
...
-- Reserve seat when booking request is submitted
...
BEGIN
  IF TG_OP = 'INSERT' AND NEW.status = 'pending' AND NEW.requested_seat_id IS NOT NULL THEN
    ...
    WHERE id = NEW.requested_seat_id AND status = 'available';
    RETURN NEW;
  END IF;
  IF TG_OP = 'UPDATE' AND NEW.requested_seat_id IS NOT NULL THEN
    IF NEW.status IN ('rejected','cancelled') AND OLD.status = 'pending' THEN
      IF NOT EXISTS (
        ...
        WHERE id = NEW.requested_seat_id AND status = 'reserved';
      END IF;
    END IF;
  END IF;
  RETURN NEW;
...
BEGIN
  IF TG_OP = 'INSERT' AND NEW.status = 'pending' AND NEW.preferred_seat_id IS NOT NULL THEN
    UPDATE seats SET status = 'reserved'
    WHERE id = NEW.preferred_seat_id AND status = 'available';
    RETURN NEW;
  END IF;
  IF TG_OP = 'UPDATE' THEN
    IF NEW.status = 'rejected' AND OLD.status = 'pending' AND OLD.preferred_seat_id IS NOT NULL THEN
      IF NOT EXISTS (
        SELECT 1 FROM seat_booking_requests
        WHERE requested_seat_id = OLD.preferred_seat_id AND status = 'pending'
      ) AND NOT EXISTS (
        SELECT 1 FROM students
        WHERE preferred_seat_id = OLD.preferred_seat_id AND status = 'pending' AND id != OLD.id
      ) THEN
        UPDATE seats SET status = 'available'
        WHERE id = OLD.preferred_seat_id AND status = 'reserved';
      END IF;
    END IF;
  END IF;
  RETURN NEW;
...
-- Track verified UTRs automatically
...
-- ── DROP ALL EXISTING FUNCTIONS FIRST (fixes return-type conflicts) ──
-- (already done at the top of this script — no-op here)
...
BEGIN
  ...
  DELETE FROM auth_lockouts WHERE locked_until < NOW() AND locked_until IS NOT NULL;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
-- ============================================================
-- SECTION 4: MATERIALIZED VIEWS (analytics)
-- ============================================================
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_monthly_membership_unique
...
-- Drop ALL existing policies (clean slate, idempotent)
DO $$ DECLARE r RECORD;
BEGIN
...
-- AUTH_LOCKOUTS — backend only (service role)
...
-- ============================================================
-- SECTION 8: BACKFILL DEFAULTS & POPULATE used_utrs
-- ============================================================

UPDATE hall_settings SET fee_reminder_days    = '[3,1]'::jsonb     WHERE fee_reminder_days    IS NULL;
...
UPDATE hall_settings SET payment_methods      = '["cash","upi"]'::jsonb WHERE payment_methods IS NULL;
...
-- ============================================================
-- SECTION 9: SEED ANALYTICS VIEWS
-- ============================================================
...
-- 34 tables + materialized views + RLS + storage + indexes
--
-- NEXT STEPS:
--   1. Go to backend/ and run: node scripts/seed-super-admin.js
--   2. Login at /super-admin/login with credentials from seed script
--   3. Create first study hall from Super Admin → Tenants → Add Hall
--   4. Login as hall admin at /admin/login and run Setup Wizard
-- ============================================================

SELECT
  'StudyHub FINAL setup complete!' AS status,
